...
const buatPesanan = async (req, res) => {
  ...
  try {
    const keranjangRes = await pool.query(
    ...
    if (keranjangRes.rows.length === 0) return res.status(400).json({ message: 'Keranjang kosong' });
    ...
    for (const item of items) {
      await pool.query(insertDetailQuery, [pesanan.id, item.menu_id, item.nama_menu, item.harga, item.foto_menu, item.jumlah, Number(item.harga) * Number(item.jumlah)]);
    }
...
const getDetailPesanan = async (req, res) => {
  try {
    ...
    if (pesananRes.rows.length === 0) return res.status(404).json({ message: 'Pesanan tidak ditemukan' });

    const detailsRes = await pool.query('SELECT * FROM pesanan_detail WHERE pesanan_id = $1', [req.params.id]);
...
const getPesananMasuk = async (req, res) => {
  ...
  try {
    ...
    // ✅ PERBAIKAN: Menambahkan `id: row.id` untuk key di frontend
    const formattedData = result.rows.map(row => ({
      id: row.id, // Kunci utama untuk React
...
const getDetailPesananMasuk = async (req, res) => {
  try {
    ...
    const data = {
      ...
      tipe_pengantaran: p.tipe_pengantaran === 'diantar' ? 'Diantar' : 'Ambil Sendiri',
